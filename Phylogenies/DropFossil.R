...
source("~/GitHubs/R/Phylogenies/CanDropFossil.R")

DropFossil <- function(newick)
{
  # Using drop.fossil
  ...
  assert(CanDropFossil(phylogeny))
  phylogeny <- drop.fossil(phylogeny)
  return (phylogeny)
}

DemonstrateCrashDropFossil <- function()
{
  # Using drop.fossil
  ...
  assert(!CanDropFossil(phylogeny_complete))
...
DemonstrateDropFossil <- function() {
  n_cols <- 1
  n_rows <- 2
  par(mfrow=c(n_rows,n_cols))
  ...
  phylogeny <- DropFossil(newick)
  plot(phylogeny,main="After")

  par(mfrow=c(1,1))
}

DemonstrateDropFossil()
DemonstrateCrashDropFossil()
